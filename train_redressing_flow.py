...
def create_coupling_dataset(args, rank, gpu, device):
    ...
    save_dir = f"./generated_couplings/{args.dataset}/seed_{args.seed}"
        ...
        with torch.no_grad():
            ...
            for j, (x0, x1) in enumerate(zip(image, noise)):
                index = (i * args.batch_size + j) * \
                    args.num_process_per_node + rank
                x0 = x0.to("cpu").numpy()
                x1 = x1.to("cpu").numpy()
...
def train(rank, gpu, args):
    ...
    if args.coupling_dir is None:
        root = create_coupling_dataset(args, rank, gpu, device)
    else:
        root = args.coupling_dir

    dataset = CouplingDataset(root)
    if args.keep_training == False:
        return
    ...
    exp = args.exp
    parent_dir = f"./saved_info/flow_matching/{args.dataset}"
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x_1, x_0) in enumerate(data_loader):
            ...
            x_r1 = x_1 - (1 - args.tau1) * \
                deep_model(torch.ones((x_1.size(0),), device=device), x_1)
            x_r0 = x_0 + args.tau0 * \
                deep_model(torch.zeros((x_1.size(0),), device=device), x_0)
            u = (x_r1 - x_r0)/(args.tau1 - args.tau0)
            x_r1 = x_r1 + (1-args.tau1) * u
            x_r0 = x_r0 - args.tau0 * u

            # sample t
            t = args.tau0+(args.tau1-args.tau0) * \
                torch.rand((x_1.size(0),), device=device)
            t = t.view(-1, 1, 1, 1)
            v_t = t * x_r1 + (1-t) * x_r0
        ...
        if rank == 0:
            ...
            for nfe in fake_samples.keys():
                torchvision.utils.save_image(
                    fake_samples[nfe],
                    os.path.join(exp_path, f'sample_epoch_{epoch}_{nfe}.png'), normalize=True)
            ...
            if epoch % args.save_ckpt_every == 0:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(exp_path, 'model_{epoch}.pth')
                )
                if args.use_ema:
                    optimizer.swap_parameters_with_ema(
                        store_params_in_ema=True)
