...
def train(rank, gpu, args):
    ...
    args.t_noise = torch.tensor(args.t_noise, device=device).view(-1, 1)
    args.t_data = torch.tensor(args.t_data, device=device).view(-1, 1)
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            b = x_1.shape[0]//2
            x_noise, x_data = x_1[:b, :, :, :], x_1[b:, :, :, :]
            model.zero_grad()
            #sample t
            t = torch.rand((x_noise.size(0),) , device=device)
            t = t.view(-1, 1, 1, 1)
            x_0 = torch.randn_like(x_noise)
            x_noise = x_noise*(1-args.t_noise) + x_0*args.t_noise
            x_0 = torch.randn_like(x_data)
            x_data = x_data*(1-args.t_data) + x_0*args.t_data
            
            v_t = (1 - t) * x_noise + t * x_data
            out = model(t.squeeze(), v_t)
            u = x_noise - x_data
                        
            # lpips_loss = lpips_model(out[:,:3,:,:], x_1)
            # lpips_loss = torch.mean(lpips_loss.reshape(lpips_loss.shape[0], -1))
