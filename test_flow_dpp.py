...
def main(args):
    ...
    ckpt = torch.load('./saved_info/flow_matching/{}/{}/model_{}.pth'.format(args.dataset, args.exp, args.epoch_id))
    ...
    def run_sampling(num_samples, generator, nfe):
        x = generator.randn(num_samples, 3, args.image_size, args.image_size).to(device)
        fake_sample = sample_from_model(model, x, nfe)[-1]
        return fake_sample
    ...
    for i in pbar:
        with torch.no_grad():
            fake_image = run_sampling(args.batch_size, generator, args.nfe)
